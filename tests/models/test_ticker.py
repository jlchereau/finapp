...
class TestTickerModel:
    ...
    def test_get_cache_path_dot_ticker(self):
        """Test cache path generation with dot ticker (European stocks)."""
        ticker = "ASML.AS"
        date = "20240101"

        cache_path = self.ticker_model._get_cache_path(ticker, date)
        expected_path = Path(self.temp_dir) / "20240101" / "ASML.AS.parquet"

        assert cache_path == expected_path

    def test_get_cache_path_london_ticker(self):
        """Test cache path generation with London ticker."""
        ticker = "ISF.L"
        date = "20240101"

        cache_path = self.ticker_model._get_cache_path(ticker, date)
        expected_path = Path(self.temp_dir) / "20240101" / "ISF.L.parquet"
    ...
    def test_get_ticker_data_from_cache(self):
        ...
        pd.testing.assert_frame_equal(
            result.reset_index(drop=True), mock_data.reset_index(drop=True)
        )
    ...
    def test_get_price_data_single_ticker(self, mock_get_ticker_data):
        ...
        mock_data = pd.DataFrame(
            {"Close": [100, 110, 120]}, index=pd.date_range("2024-01-01", periods=3)
        )
