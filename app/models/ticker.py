...
class TickerModel:
    ...
    def _get_cache_path(self, ticker: str, date: str) -> Path:
        ...
        return date_dir / f"{ticker.upper()}.parquet"
    ...
    def get_price_data(
        self, tickers: List[str], base_date: Optional[str] = None
    ) -> pd.DataFrame:
        """
        ...
        if base_date:
            try:
                ...
                if base_date_parsed in combined.index:
                    ...
                    closest_idx = combined.index.get_indexer(
                        [base_date_parsed], method="nearest"
                    )[0]
